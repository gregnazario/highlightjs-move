...
const move = require('../src/languages/move.js');
...
test('language definition shape', () => {
  const def = move(hljs);
  assert.equal(def.name, 'Move');
  assert.equal(def.unicodeRegex, true);
  assert.ok(Array.isArray(def.keywords.keyword));
  assert.ok(def.keywords.keyword.includes('module'));
  assert.ok(def.keywords.keyword.includes('fun'));
  assert.ok(def.keywords.keyword.includes('enum'));
  assert.ok(def.keywords.keyword.includes('is'));
  assert.deepEqual(def.aliases, ['move', 'aptos-move', 'move-on-aptos', 'move-lang']);
});

test('registers under all aliases', () => {
...
test('highlights declaration keywords', () => {
  // Use semicolons so declaration modes (which require an identifier name)
  // don't trigger; falls through to the keyword classifier.
  const { value } = highlight('module; fun; struct; enum; const; use; friend; public; package; native; inline; entry; script; spec; schema;');
...
test('highlights control-flow keywords', () => {
...
test('highlights ownership and reference keywords', () => {
...
test('highlights spec language keywords', () => {
...
test('Self is keyword but self is variable.language', () => {
...
test('highlights primitive types', () => {
...
test('highlights true and false as literals', () => {
...
test('highlights global storage and macro built-ins', () => {
...
test('abilities after `has` are built_in', () => {
...
test('highlights decimal numbers with and without suffix', () => {
...
test('highlights hex numbers with suffix and underscores', () => {
...
test('highlights byte strings and hex strings', () => {
...
test('highlights address literals', () => {
...
test('highlights line and block comments (block is nestable)', () => {
...
test('highlights /// doc comments with doctags', () => {
...
test('highlights attributes as meta', () => {
...
test('attribute name inside #[...] is highlighted as keyword', () => {
...
test('classifies function name after fun as title.function', () => {
  const { value } = highlight('public fun do_thing(x: u64): u64 { x }');
  const titles = textsOfKind(value, 'function');
  assert.ok(titles.includes('do_thing'), `expected do_thing as function title, got: ${titles.join(',')}`);
});

test('classifies struct and enum names (must be uppercase)', () => {
  const { value } = highlight('struct MyStruct { x: u64 } enum MyEnum { A, B }');
  const classes = textsOfKind(value, 'class');
  assert.ok(classes.includes('MyStruct'), `expected MyStruct as class title, got: ${classes.join(',')}`);
  assert.ok(classes.includes('MyEnum'), `expected MyEnum as class title, got: ${classes.join(',')}`);
});

test('classifies module path including address', () => {
  const { value } = highlight('module 0x1::shapes { }');
  const classes = textsOfKind(value, 'class');
  assert.ok(classes.some((c) => c.includes('0x1::shapes')),
    `expected module path as class title, got: ${classes.join(',')}`);
});

test('classifies qualified path like addr::module::Item', () => {
  const { value } = highlight('let x = aptos_framework::coin::CoinStore;');
  const classes = textsOfKind(value, 'class');
  assert.ok(classes.some((c) => c.includes('aptos_framework::coin::CoinStore')),
    `expected qualified path as class title, got: ${classes.join(',')}`);
});

// ---------- Negative ----------

test('non-keywords are not highlighted as keywords', () => {
  const { value } = highlight('my_var; another_thing; camelCase;');
  const kw = textsOfKind(value, 'keyword');
  assert.deepEqual(kw, []);
});

test('keyword inside identifier is not matched', () => {
...
test('Move 2 snippet: enum + match + is + attributes + assert!', () => {
  const snippet = [
    'module 0x1::shapes {',
    '    /// A shape variant',
    '    enum Shape has drop {',
